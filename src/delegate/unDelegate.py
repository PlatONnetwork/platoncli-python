...
def show_params():
    print('''
    类型      必填性         参数名称             参数解释
    int       must          staking_blocknum    质押节点时的块高
...
def unDelegate(param, address, offline, template, config, style):
    """
    this is delegate submodule unDelegate command.
    """
    if template:
        show_params()
        return
    ...
    _params = {}
    try:
        _params['staking_blocknum'] = params['staking_blocknum']
    ...
    try:
        if offline:
            data = rlp.encode(
                [rlp.encode(int(1005)), rlp.encode(_params['staking_blocknum']), rlp.encode(bytes.fromhex(_params['node_id'])),
            ...
            unsigned_file_csv_name = "unsigned_delegate_unDelegate_{}.csv".format(get_time_stamp())
            ...
            tx_result = ppos.withdrewDelegate(*_params.values())
            cust_print('delegate unDelegate send transfer transaction successful, tx result:{}.'.format(tx_result), fg='g')
    except ValueError as e:
        cust_print('delegate unDelegate send transfer transaction fail,error info:{}'.format(e), fg='r')
        sys.exit(1)
